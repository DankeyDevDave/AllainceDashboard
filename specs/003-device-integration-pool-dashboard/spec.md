# Feature Specification: eWeLink/Sonoff Device Integration for Pool Dashboard

Add support for monitoring and displaying Sono

**Feature Branch**: `003-device-integration-pool-dashboard`  
**Created**: 2025-10-05  
**Status**: Draft  
**Specification Version**: 0.0.52  
**Input**: User description: "eWeLink/Sonoff Device Integration for Pool Dashboard

Add support for monitoring and displaying Sonoff smart device power consumption (pool pump and other power monitors) in the Pool Dashboard. This integration will:

1. Allow admins to manage Sonoff devices through the admin UI (add/edit/delete devices with Home Assistant entity IDs)
2. Query InfluxDB for Sonoff device power and energy data (leveraging existing Home Assistant → InfluxDB integration)
3. Display individual device metrics on the dashboard (current power, daily energy, on/off state)
4. Calculate and display combined totals (heat pump + pool pump + other devices)
5. Show cost breakdown by device type
6. Update Grafana dashboards to include device power consumption graphs
7. Provide device status monitoring with color-coded health indicators

## Architecture Approach

**Use Home Assistant → InfluxDB Integration (Recommended)**
- Leverages existing Home Assistant setup with eWeLink integration
- Data flows through existing InfluxDB pipeline (unified data source)
- No additional API calls or credential management needed
- Automatic data logging and retention
- Existing Grafana dashboards can be extended
- More reliable and battle-tested than direct eWeLink API

## User Value

- **Complete System Visibility**: See total pool system power consumption (heat pump + pump + accessories)
- **Cost Accuracy**: Accurate electricity cost calculations including all pool equipment
- **Device Management**: Easy configuration of monitored devices through admin UI
- **Trend Analysis**: Historical data for all devices in Grafana dashboards
- **Troubleshooting**: Quickly identify devices with unusual power consumption or offline status
- **Energy Optimization**: Identify opportunities to reduce pool system energy usage

## Technical Requirements

1. Device Management UI in admin settings
2. InfluxDB query functions for Sonoff device entities
3. Combined metrics calculations (totals and breakdowns)
4. Dashboard UI updates for device display
5. API endpoints for device data
6. Type definitions for device data structures
7. Grafana dashboard updates for device graphs
8. Documentation for Home Assistant eWeLink setup

## Out of Scope

- Direct eWeLink API integration (using HA → InfluxDB instead)
- Device control/switching (read-only monitoring for now)
- Real-time device notifications/alerts
- Historical trend analysis beyond Grafana (future enhancement)
